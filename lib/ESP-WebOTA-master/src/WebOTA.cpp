...
<head>
	...
	<title>Weighing Batcher</title>

	<script src="main.js"></script>
	<link rel="stylesheet" href="main.css">
</head>
<body>
	<h1>Weighing Batcher %s</h1>
...
int WebOTA::add_http_routes(WebServer *server, const char *path) {
	...
	server->on("/", HTTP_GET, [server]() {
		...
		server->send(200, "text/html", F("<h1 style=\"font-family: sans-serif;\">OTA Mode</h1>"));
	});
	...
	server->on(path, HTTP_GET, [server,this]() {
		...
		if (this->custom_html != NULL) {
			...
			String board_type = "Weighing Batcher";

			char buf[1024];
			snprintf_P(buf, sizeof(buf), INDEX_HTML, WEBOTA_VERSION, board_type.c_str(), uptime_str.c_str());
